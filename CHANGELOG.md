...
- **Codex chat/sidebar monitoring by default** - Pixel Agents adopts active OpenAI VS Code Codex chat/sidebar sessions from `~/.codex/sessions`.
- **Hooks-first session management with dual-mode architecture** ([#214](https://github.com/pablodelucca/pixel-agents/pull/214)) — Splits agent detection into a preferred hooks path and a heuristic fallback. When Codex hooks are available, session lifecycle, tool activity, permissions, and sub-agent events are reported instantly via a local HTTP server; when unavailable, the extension transparently falls back to JSONL polling. Builds on [#187](https://github.com/pablodelucca/pixel-agents/pull/187). Closes [#188](https://github.com/pablodelucca/pixel-agents/issues/188), [#201](https://github.com/pablodelucca/pixel-agents/issues/201).
- **Codex hooks for instant agent status detection** ([#187](https://github.com/pablodelucca/pixel-agents/pull/187)) — Adds a standalone HTTP server and hook installer that routes Codex hook events to the webview for sub-second status updates, replacing filesystem polling when hooks are installed.
- **External session support and Agent tool recognition** ([#115](https://github.com/pablodelucca/pixel-agents/pull/115)) — Detects Codex sessions launched outside the extension (external CLIs, other editors) and recognizes agent-spawning tool activity so sub-agent characters spawn correctly. Closes [#184](https://github.com/pablodelucca/pixel-agents/issues/184), [#74](https://github.com/pablodelucca/pixel-agents/issues/74), [#9](https://github.com/pablodelucca/pixel-agents/issues/9), [#8](https://github.com/pablodelucca/pixel-agents/issues/8), [#1](https://github.com/pablodelucca/pixel-agents/issues/1). Supersedes [#2](https://github.com/pablodelucca/pixel-agents/pull/2), [#77](https://github.com/pablodelucca/pixel-agents/pull/77), [#101](https://github.com/pablodelucca/pixel-agents/pull/101), [#141](https://github.com/pablodelucca/pixel-agents/pull/141).
...
- **New generated office assets** - Adds two animated computer variants, two chair variants, an executive desk, and six desk accessories matching the built-in pixel furniture style.

### Fixes

- **Launch Codex from the OpenAI VS Code extension bundle** - Pixel Agents now resolves the Codex CLI bundled with the OpenAI Codex/ChatGPT VS Code extension before falling back to terminal `PATH`, fixing setups where `codex` is not globally installed.
- **Read newer Codex chat metadata** - Session detection now handles nested `payload` metadata and large first-line records written by the OpenAI VS Code extension.
- **Keep spawned Codex agents alive until close** - `spawn_agent` children now stay visible after spawn acknowledgement and are removed when Codex emits the matching `close_agent` / `collab_close_end` event.
- **Prevent duplicate spawned Codex agents** - Monitor Codex Chat and hook adoption now skip child session JSONL files already owned by a parent `spawn_agent`, so six spawned agents render as six children instead of extra top-level agents.
- **Ignore incomplete Codex spawn attempts** - `spawn_agent` calls now become persistent spawned children only after Codex emits a `collab_agent_spawn_end` with a real child thread id, preventing phantom children from failed or partial spawn attempts.
- **Remove toolbar agent launch button** - The bottom toolbar now shows only layout and settings controls; Codex sessions are started directly in chat/sidebar or terminal and adopted by monitoring.
- **Harden Codex session lifecycle review findings** - Installed the full set of consumed Codex hook events, preserved spawned children through hook Stop/restore paths, protected pending terminal sessions from global adoption, and fixed nested-session E2E discovery.
...
- [@pablodelucca](https://github.com/pablodelucca), [@NNTin](https://github.com/NNTin) — Codex hooks integration, Tailwind v4 migration
...
- **Bypass permissions mode** ([#170](https://github.com/pablodelucca/pixel-agents/pull/170)) — Launch controls can request `--dangerously-skip-permissions`, skipping all tool-call approval prompts.
...
- **Agents not appearing on Linux Mint and macOS without a folder open** ([#70](https://github.com/pablodelucca/pixel-agents/pull/70)) — Falls back to `os.homedir()` when no workspace folder is open, matching Codex behavior.

### Testing

- **Playwright e2e tests** ([#161](https://github.com/pablodelucca/pixel-agents/pull/161)) — End-to-end test infrastructure using Playwright's Electron API with a mock Codex CLI, validating agent spawn flow in a real VS Code instance.
...
- **Recognize agent-spawning tool names for sub-agent visualization** ([#76](https://github.com/pablodelucca/pixel-agents/pull/76)) — Sub-agent characters now spawn correctly with current Codex tool events.
...
- **Workspace folder picker for multi-root workspaces** ([#12](https://github.com/pablodelucca/pixel-agents/pull/12)) — Agent launch controls can choose which workspace folder to open Codex in.
